...
function MapPopup({
    ...
    return (
        <LeafletPopup
            className={cn(
                "bg-popover text-popover-foreground animate-in fade-out-0 fade-in-0 zoom-out-95 zoom-in-95 slide-in-from-bottom-2 z-50 w-max rounded-md border p-4 font-sans shadow-md outline-hidden",
                className
            )}
            {...props}
        />
    ...
    return (
        <>
            <Button
                ...
                className={cn(
                    "absolute right-1 bottom-1 z-1000 border bg-background",
                    className
                )}
                {...props}>
                {isLocating ? (
...
function useLeaflet() {
    // Return the statically imported Leaflet library
    // LeafletDraw is imported as a side effect and extends L
    return { L, LeafletDraw: L }
}
